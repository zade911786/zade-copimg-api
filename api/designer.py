...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ── FEATURE: Image Serving Route ──────────────────────────
        # Agar URL mein '?file=imagename.jpg' hai, toh use /tmp se read karke serve karo
        if "file" in qs:
            filename = os.path.basename(qs["file"][0].strip()) # Safety check against path traversal
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    self._json(500, {"success": False, "message": f"Error serving file: {str(serve_err)}"})
                    return
            else:
                self._json(404, {"success": False, "message": "Image expired or not found. Serverless tmp files clear frequently."})
                return
        ...
        if create_img is None:
            self._json(500, {
                ...
                "message": "microsoftdesigner package not installed.",
            })
            return
        ...
        # Host dynamic handle karne ke liye (Vercel domain fetch)
        ...
        try:
            ...
            if not image_paths:
                self._json(403, {
                    ...
                    "message": "Designer returned no images.",
                    "elapsed_seconds": elapsed,
                })
                return

            # Local path se filename nikal kar public API links banana
            images = []
            for path in image_paths:
                filename = os.path.basename(path)
                # Dynamic serve link generation
                file_url = f"{base_url}?file={filename}"
                images.append({
                    "url": file_url,
                    "filename": filename
                })
                # Note: Ab hum file ko turant delete nahi kar rahe hain taaki link work kare.
                # Vercel container automatic clean up ho jata hai thodi der baad.
            ...
            if "403" in err or "credit" in err.lower():
                ...
                msg  = "Auth token expired or credits exhausted."
            elif "401" in err or "auth" in err.lower() or "token" in err.lower():
                code = "AUTH_FAILED"
                msg  = "Invalid USER_ID or AUTH_TOKEN."
            else:
                code = "INTERNAL_FAULT"
